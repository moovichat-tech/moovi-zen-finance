...
const ExpensesPage = () => {
  ...
  const { open, setOpen, openAdd } = useTransactionForm('expense');
  ...
  const { data: despesas = [], isLoading } = useQuery<Despesa[]>({
    queryKey: ['despesas', queryPayload],
    queryFn: async () => {
      if (!token) return [];
      const res = await fetch(`${SUPABASE_URL}/functions/v1/get-despesas`, {
        method: 'POST',
  ...
  const statusLabel = (s: string) => {
    if (s === 'PAGO') return t.common.paid;
  ...
  return (
    <div className="space-y-4 sm:space-y-6 animate-in-up">
                ...
                <TableRow key={exp.id}>
                  <TableCell className="font-medium">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="truncate max-w-[120px] sm:max-w-none">{exp.descricao}</span>
                    </div>
                  </TableCell>
                  <TableCell className="text-sm text-muted-foreground whitespace-nowrap">{exp.categoria}</TableCell>
      ...
      <TransactionFormDialog type="expense" open={open} onOpenChange={setOpen} />
