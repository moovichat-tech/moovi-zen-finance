...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const descricao = typeof body?.descricao === "string" && body.descricao.trim()
      ? body.descricao.trim()
      : null;
    const dataHora = body?.data_hora_limite;
    ...
    const rows = await sql`
      INSERT INTO compromissos (telefone_usuario, titulo, descricao, data_hora_limite, status)
      VALUES (${telefone}, ${titulo}, ${descricao}, ${new Date(dataHora).toISOString()}, 'pendente')
      RETURNING id, telefone_usuario, titulo, descricao, data_hora_limite, status
    `;
    ...
    console.error("compromissos-create error:", e);
