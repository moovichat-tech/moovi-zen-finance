...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const { id, nome, valor_guardado, valor_meta } = await req.json();

    if (!id || !nome || valor_meta == null || valor_guardado == null) {
      throw new Error("Campos obrigatórios: id, nome, valor_guardado, valor_meta");
    }

    await sql`
      UPDATE metas
      SET nome = ${nome},
          valor_guardado = ${Number(valor_guardado)},
