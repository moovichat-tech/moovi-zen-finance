...
const ReportsPage = () => {
  ...
  const monthlyData = useMemo(() => {
    ...
    return months.map((month, i) => {
      ...
      const inc = transactions.filter(tr => tr.type === 'income' && tr.date.includes(`-${monthStr}-`)).reduce((s, t) => s + t.amount, 0);
      const exp = transactions.filter(tr => tr.type === 'expense' && tr.date.includes(`-${monthStr}-`)).reduce((s, t) => s + t.amount, 0);
  ...
  const exportCSV = () => {
    ...
    const rows = transactions.map(tr => `${tr.type},${tr.description},${tr.amount},${tr.category},${tr.date},${tr.status}`).join('\n');
